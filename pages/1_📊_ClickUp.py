...
apply_page_style("ClickUp · Dashboard", icon="📊")

if not os.path.exists(REPORT_PATH):
    st.error(
        f"No se encontró `{os.path.basename(REPORT_PATH)}`. "
        "Corré primero `python clickup_tasks.py` para generar el reporte."
    )
    st.stop()

df = load_clickup(REPORT_PATH, os.path.getmtime(REPORT_PATH))
...
render_header(
    ...
    refresh_scripts=["clickup_tasks.py"],
    button_key="refresh_clickup",
)

# ── KPIs ──────────────────────────────────────────────────────────────────────
total_tasks = len(filtered)
closed_tasks = int((filtered["date_closed"].notna()).sum()) if "date_closed" in filtered else 0
open_tasks_n = total_tasks - closed_tasks
...
c2.metric("Abiertas", f"{open_tasks_n:,}")
...
# ── TABS ──────────────────────────────────────────────────────────────────────
tab_overview, tab_people, tab_status, tab_struct, tab_list, tab_table, tab_msp = st.tabs(
    [
        "🏠 Resumen",
        "👥 Por persona",
        "📌 Por status",
...
with tab_msp:
    st.subheader("Cuentas activas MSP")
    try:
        msp = load_msp_accounts(MSP_SHEET_ID)
    except Exception as e:
        st.error(
            "No se pudo leer el Google Sheet. Verificá que esté compartido como "
    ...
    if msp.empty:
        st.info("No hay cuentas marcadas como MSP=Sí.")
    else:
        st.metric("Cuentas activas", len(msp))
        search = st.text_input("Buscar", placeholder="Filtrar por cualquier columna...")
        view = msp
        if search:
            mask = view.apply(
        ...
        csv = msp.to_csv(index=False).encode("utf-8")
        st.download_button(
            "⬇️ Descargar CSV",
            data=csv,
            file_name="cuentas_activas_msp.csv",
            mime="text/csv",
        )
